...
%     iflimup(9,:) = iflimup(9,:)/8750*8450;
    ...
    %temperature, decompose load to each bus with the factor in NYgrid model
        ...
        evload(FIev,:) = evload(FIev,:)*ev_rateFI;
        evload(JKev,:) = evload(JKev,:)*ev_rateJK;
        ...
        Buldingload(FIbd,:) = Buldingload(FIbd,:)*bd_rateFI;
        Buldingload(JKbd,:) = Buldingload(JKbd,:)*bd_rateJK;
...
%         OO = sum(sum(loadshedding));
    ...
    directory_path = 'Baseline_v4/Scenario'+string(lhscenario);
    if ~exist("directory_path",'dir')
        mkdir(directory_path)
    end
    % writematrix(result,'lmpwre.csv')
    writematrix(genresult,'Baseline_v4/Scenario'+string(lhscenario)+'/gen_'+string(year)+'.csv')
    writematrix(flowresult,'Baseline_v4/Scenario'+string(lhscenario)+'/flow_'+string(year)+'.csv')
    writematrix(ifsumresult,'Baseline_v4/Scenario'+string(lhscenario)+'/ifsum_'+string(year)+'.csv')
    writematrix(chargeresult,'Baseline_v4/Scenario'+string(lhscenario)+'/charege_'+string(year)+'.csv')
    writematrix(dischargeresult,'Baseline_v4/Scenario'+string(lhscenario)+'/disch_'+string(year)+'.csv')
    writematrix(curtailwind,'Baseline_v4/Scenario'+string(lhscenario)+'/wc_'+string(year)+'.csv')
    writematrix(curtailsolar,'Baseline_v4/Scenario'+string(lhscenario)+'/sc_'+string(year)+'.csv')
    % writematrix(curtailhydro,'Baseline_v4/hcwre'+string(testcase)+'.csv')
    % writematrix(lmpresult,'Baseline_v4/lmpwre'+string(testcase)+'.csv')
    writematrix(battstateresult,'Baseline_v4/Scenario'+string(lhscenario)+'/battstate_'+string(year)+'.csv')
    writematrix(loadshedresult,'Baseline_v4/Scenario'+string(lhscenario)+'/loadshed_'+string(year)+'.csv')
end